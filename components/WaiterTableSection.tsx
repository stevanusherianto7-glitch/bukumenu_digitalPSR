...
export const WaiterTableSection: React.FC<{ onExit?: () => void }> = ({ onExit }) => {
    ...
    // Data bersumber dari Global Store (OrderManager yang melakukan polling)
    // Filter orders yang valid (array check)
    const safeOrdersCombined = Array.isArray(orders) ? orders : [];

    // Generate Tables A1-A9
    const tables = Array.from({ length: 9 }, (_, i) => `A${i + 1}`);

    const getTableOrders = (tableNum: string) => {
        return safeOrdersCombined.filter(o => o.tableNumber === tableNum && o.status === 'pending');
    };

    // Ambil history order SPESIFIK untuk meja tertentu
    const getTableHistory = (tableNum: string) => {
        return safeOrdersCombined
    ...
    const globalHistoryOrders = safeOrdersCombined
        .filter(o => o.status === 'completed')
        .sort((a, b) => b.timestamp - a.timestamp);

    const activeTablesCount = new Set(safeOrdersCombined.filter(o => o.status === 'pending').map(o => o.tableNumber)).size;
