...
export const WaiterTableSection: React.FC<{ onExit?: () => void }> = ({ onExit }) => {
  const { orders, completeOrder, setOrders } = useOrderStore();
  ...
  useEffect(() => {
    // Fungsi untuk mengambil data pesanan pending dari server
  ...
  }, [setOrders]); // Dependency array, setOrders dijamin stabil oleh Zustand
  ...
  const getTableHistory = (tableNum: string) => {
      // NOTE: History tidak di-fetch real-time, jadi ini akan kosong
      // Perlu endpoint baru untuk history jika fitur ini penting
      return [];
  };
  ...
  const handleSimulateOrder = async () => {
    ...
    try {
        await api.post('/orders', {
            ...
            items: [{ ...randomItem, quantity: randomQuantity, notes: randomNote }]
        });
        ...
        <div className="bg-gray-900 text-white p-6 pb-8 rounded-b-[40px] shadow-xl shadow-gray-900/20 sticky top-0 z-30 relative overflow-hidden -mt-0">
           ...
           <div className="relative z-10">
               <div className="flex items-center justify-between mb-6">
                    <button onClick={() => setSelectedTable(null)} className="flex items-center gap-3 text-white/90 hover:text-white font-bold active:scale-95 transition-transform group">
               ...
               <div className="flex bg-white/10 p-1.5 rounded-xl border border-white/10 backdrop-blur-md">
                    <button onClick={() => setTableDetailTab('active')} className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${tableDetailTab === 'active' ? 'bg-white text-gray-900 shadow-lg' : 'text-white/60 hover:bg-white/5 hover:text-white'}`}>
                    ...
                    <button onClick={() => setTableDetailTab('history')} className={`flex-1 py-2.5 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-2 ${tableDetailTab === 'history' ? 'bg-white text-gray-900 shadow-lg' : 'text-white/60 hover:bg-white/5 hover:text-white'}`}>
                ...
                {activeOrders.length === 0 ? (
                    ...
                    <div className="space-y-4">
                        ...
                        <div key={order.id} className="bg-white border border-gray-100 rounded-2xl p-5 shadow-[0_4px_20px_rgba(0,0,0,0.03)] relative overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
                            ...
                            {order.items.map((item, idx) => (
                                ...
                                <div className="flex gap-3 w-full">
                                    ...
                                    {item.notes && item.notes.trim() !== '' && (
                                        ...
                                        <div className="flex-1">
                                            <p className="text-[10px] font-bold text-yellow-700 uppercase tracking-wide mb-0.5 font-sans">Catatan Khusus:</p>
                                            <p className="text-sm text-gray-800 font-medium italic leading-snug break-words font-sans">"{item.notes}"</p>
                                        </div>
                                        </div>
                                    )}
                                    </div>
                            ...
                            <button onClick={async () => { if(window.confirm('Tandai pesanan ini sudah diantar/selesai?')) { await completeOrder(order.id); } }} className="w-full bg-green-600 text-white font-bold py-3.5 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-green-600/20 hover:bg-green-700 active:scale-[0.98] transition-all font-sans">
                            ...
                            <div className="mt-3 flex items-start gap-2 bg-orange-50 p-3 rounded-lg border border-orange-100">
                                ...
                                <p className="text-[10px] text-orange-800 font-medium font-sans italic leading-snug">Segera lakukan repeat order kepada pelanggan untuk memastikan ulang bahwa pesanan sudah benar.</p>
                            </div>
                            <div className="mt-2 flex items-start gap-2 bg-blue-50 p-3 rounded-lg border border-blue-100">
                                <Info size={14} className="text-blue-600 shrink-0 mt-0.5" />
                                <p className="text-[10px] text-blue-800 font-medium font-sans italic leading-snug">Perhatikan catatan khusus pelanggan sebelum menyajikan makanan.</p>
                            </div>
                            </div>
                        </div>
                        ))}
           ...
           {tableDetailTab === 'history' && (
               <div className="space-y-4">
                   <div className="text-center py-20 text-gray-400">
                       ...
                       <p className="text-xs">Fitur riwayat per meja akan dikembangkan.</p>
                   </div>
               </div>
           )}
        </div>
    ...
    <div className="animate-in fade-in pb-20">
      <div className="bg-gray-900 text-white p-6 pb-12 rounded-b-[40px] shadow-xl shadow-gray-900/10 mb-4 relative overflow-hidden mx-0 -mt-2">
        ...
        <div className="relative z-10 bg-white/10 p-1 rounded-xl flex backdrop-blur-sm border border-white/10">
            <button onClick={() => setActiveTab('monitor')} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'monitor' ? 'bg-white text-gray-900 shadow-md' : 'text-white/70 hover:bg-white/5'}`}>
                <LayoutGrid size={16} /> Monitor Meja
            </button>
            <button onClick={() => setActiveTab('history')} className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-bold transition-all ${activeTab === 'history' ? 'bg-white text-gray-900 shadow-md' : 'text-white/70 hover:bg-white/5'}`}>
        ...
        <>
          ...
          <div className="px-6 space-y-4">
              <div className="text-center py-20 text-gray-400">
                  ...
                  <p className="font-bold">Fitur Riwayat Global</p>
                  <p className="text-xs">Akan dikembangkan lebih lanjut.</p>
              </div>
          </div>
      )}
    </div>
